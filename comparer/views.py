    ...
    if request.method == "POST":
        try:
            ...
            if not file1 or not file2:
                return JsonResponse(
                    {"status": "error", "message": "Both files are required"}
                )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    return JsonResponse(
                        {"status": "error", "message": "Invalid field mapping format"},
                        status=400,
                    )
    ...
    if request.method == "POST":
        try:
            ...
            mapping = service.add_field_mapping_db(
                field_type=data["field_type"], variations=data["variations"]
            )
...
def delete_mapping_view(request):
    try:
        ...
        return JsonResponse(
            {"status": "success", "message": "Mapping deleted successfully"}
        )
    except json.JSONDecodeError:
...
def debug_mappings(request):
    ...
    return JsonResponse(
        {
            "mappings": [
                {"id": m.id, "field_type": m.field_type, "variations": m.variations}
                for m in mappings
            ]
        }
    )
...
def result_view(request):
    ...
    try:
        ...
        context = {
            ...
            "results": {
                "total_rows": {"file1": comparison.file1, "file2": comparison.file2},
...
def dashboard_view(request):
    ...
    recent_comparisons = ComparisonHistory.objects.filter(user=request.user).order_by(
        "-comparison_date"
    )[:10]  # Show 10 most recent comparisons
    ...
    for comp in recent_comparisons:
        activity = {
            ...
            "type": "Scheduled Task"
            if hasattr(comp, "scheduled_task")
            else "Manual Comparison",
    ...
    context = {
        "subscription": {
            ...
            if user_stats.subscription_type == "Free"
            else 1000,
        },
        "user_stats": {
